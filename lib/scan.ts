...
import { pointInPolygon, Point } from "./geometry-utils";
...
export async function scanDocumentForComponents(
  ...
  if (options.enableGeometry) {
    ...
    // If preview polygon provided, use it
    ...
    // Assign system based on geometry
    for (const component of components) {
      ...
      for (const polygon of pagePolygons) {
        ...
        if (isInside) {
          // Override system with polygon system
          component.system = polygon.systemCode;
          geometryAssigned++;
          break; // Use first matching polygon
        }
      }
    }
...
/**
 * Verify scanned components against mass list
 * Returns components missing in drawing and components missing in mass list
 */
...
/**
 * Scan document text for components using simple text parsing
 * Used for document verification endpoint
 */
