...
// System code: 3-4 siffer, punkt, 3-4 siffer, valgfri kolon-suffiks
// Eksempel: 360.0001, 360.001, 5640.0001, 360.001:02
const SYSTEM_RE = /(?:\+(?<byggnr>[A-Za-z0-9]+)=)?=?(?<system>\d{3,4}\.\d{3,4}(?::\d{2,4})?)/gi;

// Inline TFM: Komplett {byggnr}{system}{komponent}{typekode} i én token
// Byggnr: + etterfulgt av siffer/bokstaver
// System: 3-4 siffer . 3-4 siffer, valgfri :versjon
// Komponent: 2-3 bokstaver etterfulgt av siffer (kan splittes av spesialtegn)
// Typekode: % etterfulgt av 2-3 bokstaver
const INLINE_TFM_RE = /(?:\+(?<byggnr>[A-Za-z0-9]+)=)?=?(?<system>\d{3,4}\.\d{3,4}(?::\d{2,4})?)\s*[-\s]*\s*(?<komponent>[A-Z]{2,3}\d+[0-9A-Z/_\-]*)(?:%(?<typekode>[A-Z]{2,3}))?/gi;

// Component: 2-3 bokstaver + minst ett siffer + valgfrie alfanumeriske/spesialtegn
// Eksempel: RTA4001, KA001, AA01T/003
const COMPONENT_RE = /\b(?<komponent>[A-Z]{2,3}\d+[0-9A-Z/_\-]*)\b/gi;

// Typecode: % + 2-3 bokstaver
