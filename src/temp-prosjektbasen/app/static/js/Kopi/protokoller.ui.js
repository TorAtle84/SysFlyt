...
async function handleGeneration() {
  ...
  try {
    if (funksjon.toUpperCase() === "MC") {
      ...
      const finalRows = Array.isArray(data) ? data : (data.rows || []);
      window.allRows = finalRows;
      ...
      if (funksjon.toUpperCase() === "FUNKSJONSTEST") {
        const tfms = [...new Set(finalRows.map(r => (r.komponent || "").substring(0, 2).toUpperCase()))].filter(Boolean);
        if (tfms.length) {
          try {
      ...
      renderFunction(finalRows);
      protocolTable.style.display = "table";
      if (statusText) statusText.textContent = `Fant ${finalRows.length} komponenter.`;
