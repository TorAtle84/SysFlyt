...
MASTER_REGEX_GENERIC = re.compile(
    ...
    r"(?:=(?P<system>[\d.:/]+))?"                # =SYSTEM (tolerant)
    r"(?:-?(?P<komponent>[A-Za-z]{2,3}\d{3,4}))?"# -XX000 / XX000 / -XXX0000
...
MASTER_REGEX_PDF = re.compile(
    ...
    r"(?:-?(?P<pdf_komponent>[A-Za-z]{2,3}\d{3,4}))?"    # -XX000 / XX000 / -XXX0000
        ...
        for filename, file_bytes in files_mem:
            ...
            try:
                ...
                for row in parse_rows_from_text(text, filename):
                    ...
                    # --- NYTT: finn komponent robust
                    komponent = (row.get("komponent") or
                                 parse_component_from_full_id(row.get("full_id", "")) or
                                 "")

                    # --- NYTT: slå opp TFM på de to første bokstavene av komponenten
