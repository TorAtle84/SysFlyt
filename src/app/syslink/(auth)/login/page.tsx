    ...
    try {
      ...
      if (checkData.success) {
        ...
        if (res?.error) {
          ...
          router.replace("/syslink/dashboard");
        }
      }
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {!showTotp && (
        <p className="text-sm text-muted-foreground">
          Har du ikke bruker?{" "}
          <Link href="/syslink/register" className="font-semibold text-info underline-offset-4 hover:underline">
            Opprett konto
          </Link>
          {" • "}
          <Link href="/syslink/reset" className="font-semibold text-info underline-offset-4 hover:underline">
            Glemt passord?
          </Link>
        </p>
      )}
