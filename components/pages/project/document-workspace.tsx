...
export function DocumentWorkspace({
  ...
  const handleScanSystems = useCallback(async (documentId: string) => {
    setScanningSystemDocId(documentId);
    try {
      const res = await fetch(
        `/api/projects/${project.id}/documents/${documentId}/systems`,
        { method: "POST" }
      );
                    ...
                    <TableCell>
                      <div className="flex justify-end gap-1">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleScanSystems(doc.id)}
                          disabled={scanningSystemDocId === doc.id}
                          title="Skann systemnummer i PDF"
                        >
                          {scanningSystemDocId === doc.id ? (
                ...
                {systemScanResult.systems.length > 0 ? (
                  <>
                    ...
                    <p className="text-muted-foreground">
                      Sortert etter plassering i dokumentet (øverst = hovedsystem). Systemtagger er oppdatert.
                    </p>
                  </>
                ) : (
                  <p className="text-muted-foreground">
                    Ingen systemkoder funnet i dokumentet.
                  </p>
                )}
              </div>
              ...
              {systemScanResult.systems.length > 0 && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs uppercase text-muted-foreground">
                    ...
                    <span>{systemScanResult.systems.length} stk</span>
                  </div>
                  <div className="max-h-64 space-y-2 overflow-y-auto">
                    {systemScanResult.systems.map((sys, idx) => (
                      ...
                      >
                        <div>
                          <div className="font-mono text-sm">{sys.code}</div>
                          <div className="text-xs text-muted-foreground">
                            Side {sys.page} • y={sys.y.toFixed(1)}%
